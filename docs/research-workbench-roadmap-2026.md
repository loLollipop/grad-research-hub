...
笔记页已经具备 Markdown、大编辑区、`[[双链]]`、反向链接、清单拆任务和写作素材包。新的优化重点不是引入复杂编辑器或知识图谱，而是让用户打开笔记页时立刻知道哪些碎片该收口。

本轮调整：

1. 左侧新增“沉淀雷达”，把可拆任务、待补双链、写作素材、收件箱四类信号放在一起。
2. 每条最近笔记增加行动理由，解释为什么要拆任务、补双链、继续打磨或继续写。
3. 继续复用现有笔记正文、分类、双链和清单解析能力，不新增数据库表。
4. 笔记页继续保留右侧大编辑区，不把写作变成弹窗或后台表单。

验收标准：

- 用户打开笔记页后，能看到当前最值得沉淀的笔记类型。
- 带待办清单的笔记会被明确引导拆回任务。
- 有断链主题的笔记会被提示补双链，而不是要求维护复杂知识图谱。
- 写作相关笔记会被引导进入组会、周报或论文草稿。
- `npm run lint` 和 `npm run build` 通过。

## 本轮追加落地：事务减负雷达

事务页已经覆盖组会、材料、报销和硬截止，但它不应该演变成完整行政审批系统或复杂日历。新的优化重点是减少研究生在群消息、邮件、学院系统之间来回翻找的心理负担。

本轮调整：

1. 左侧新增“减负雷达”，把今天/逾期、组会准备、报销材料、材料/截止四类高打断事项放在一起。
2. “现在最该处理”和每张事务卡增加行动理由，解释为什么应先补逾期、收口今天截止、继续推进或准备组会材料。
3. 雷达计数基于全量未完成事务，不受当前筛选影响，避免用户进入某个分类后误以为其他事务不存在。
4. 继续复用现有事务类型、状态、截止日期和备注字段，不新增数据库表，不做审批流、复杂日历或组织级权限。

验收标准：

- 用户打开事务页后，能直接看到会打断科研的事项集中在哪一类。
- 今天到期和逾期事务会被优先提示，而不是混在普通列表里。
- 组会、报销、材料类事务都有明确下一步说明。
- `npm run lint` 和 `npm run build` 通过。
